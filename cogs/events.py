...
class EventScheduler(commands.Cog):
    ...
    async def _run_event_cycle(
        ...
        try:
            ...
            # 1 hour before
            one_hour_before = ev.start_epoch - 60*60
            reminder_id = None
            if now < one_hour_before:
                await asyncio.sleep(one_hour_before - now)
                reminder_id = await self._send_event_ping(ch, guild_cfg, 60)
                guild_cfg.setdefault("event", {})["reminder_id"] = reminder_id
                save_config(gcfg)
                now = int(time.time())
            # 10 minutes before
            ten_min_before = ev.start_epoch - 10*60
            if now < ten_min_before:
                await asyncio.sleep(ten_min_before - now)
                # Delete 1 hour reminder
                ...
                reminder_id = await self._send_event_ping(ch, guild_cfg, 10)
                guild_cfg.setdefault("event", {})["reminder_id"] = reminder_id
                save_config(gcfg)
                now = int(time.time())
            ...
            # Delete 10 minute reminder at event start
            reminder_id = guild_cfg.get("event", {}).get("reminder_id")
            if reminder_id:
                try:
                    msg = await ch.fetch_message(reminder_id)
    ...
    async def create_event(self, interaction, title, description, s_epoch, e_epoch, thumbnail, template_key=None):
        ...
        if soonest_entry["id"] == new_id:
            ...
            ev.task = asyncio.create_task(self._run_event_cycle(guild, ev, ch))
            # If the event is already within the 1 hour or 10 minute window, send the appropriate notification immediately
            now = int(time.time())
            one_hour_before = s_epoch - 60*60
            ten_min_before = s_epoch - 10*60
            if now >= one_hour_before and now < ten_min_before:
                reminder_id = await self._send_event_ping(ch, guild_cfg, 60)
                guild_cfg.setdefault("event", {})["reminder_id"] = reminder_id
                save_config(gcfg)
            elif now >= ten_min_before and now < s_epoch:
                reminder_id = await self._send_event_ping(ch, guild_cfg, 10)
                guild_cfg.setdefault("event", {})["reminder_id"] = reminder_id
                save_config(gcfg)
